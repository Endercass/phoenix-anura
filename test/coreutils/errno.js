...
export const runErrnoTests = () => {
  describe("errno", function () {
    const testCases = [
      ...
      {
        description:
          "prints an error message and returns a code > 0 if an error is not found",
      ...
      {
        description:
          "lists all errors if --list is provided, ignoring parameters",
        input: ["directory"],
        values: { list: true },
        expectedStdout:
    ...
    for (const {
      ...
      it(description, async () => {
        let ctx = MakeTestContext(builtins.errno, {
          positionals: input,
          values,
        });
        let hadError = false;
