...
export const runPrintfTests = () => {
  describe("printf", function () {
    const testCases = [
      ...
      {
        description:
          "outputs an unsigned hexadecimal integer for `%x` and `%X`",
        input: ['"%x" "%X"\n', "13", "13", "0", "0", "-127", "-127"],
        expectedStdout: '"d" "D"\n"0" "0"\n"ffffff81" "FFFFFF81"\n',
        expectedStderr: "",
      },
      ...
      {
        description:
          "ensures a starting `0x` or `0X` when using the `#` flag for `%x` and `%X`",
        input: ['"%#x" "%#0X"\n', "13", "13", "0", "0", "-127", "-127"],
      ...
      {
        description:
          "outputs a floating point number in decimal notation for `%f` and `%F`",
        input: [
          '"%f" "%F"\n',
          "13",
          "13",
      ...
      {
        description:
          "zero precision with `#` flag forces a decimal point for `%f` and `%F`",
        input: [
          '"%#.0f" "%0#.0F"\n',
          "13",
          "13",
      ...
      {
        description:
          "outputs a floating point number in exponential notation for `%e` and `%E`",
        input: [
          '"%e" "%E"\n',
          "13",
          "13",
      ...
      {
        description:
          "zero precision with `#` flag forces a decimal point for `%e` and `%E`",
        input: [
          '"%#.0e" "%0#.0E"\n',
          "13",
          "13",
      ...
      {
        description:
          "zero precision with `#` flag forces a decimal point for `%g` and `%G`",
        input: [
          '"%#.0g" "%0#.0G"\n',
          "13",
          "13",
    ...
    for (const {
      ...
      it(description, async () => {
        ...
        try {
          ...
          if (!expectedFail) {
            assert.equal(
              result,
              undefined,
              "should exit successfully, returning nothing",
            );
          }
        } catch (e) {
          hadError = true;
        ...
        assert.equal(
          ...
          expectedStdout,
          "wrong output written to stdout",
        );
        assert.equal(
          ctx.externs.err.output,
          expectedStderr,
          "wrong output written to stderr",
        );
      });
    }
  });
